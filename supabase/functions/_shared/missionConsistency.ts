...
[이유 선택지의 구별]
primary만 현재 표현의 실제 주원인을 설명한다. pragmatic_misconception은 그럴듯하지만
현재 문구·관계에 비추어 틀린 화용 해석, meaning_grammar_context는 실제 문구에서
연상할 수 있지만 원문·문법·상황과 대조하면 반박되는 별개의 오진을 제시한다.
세 선택지 모두 이유를 주장해야 한다. "문법적으로 문제가 없다"만으로 끝내지 않는다.
"직접적이다", "선택권이 부족하다", "더 완화해야 한다"가 이 문항에서 같은 진단이라면
세 선택지로 나누지 않는다. 오답이 주원인의 바꿔 말하기·부분 설명이면 다시 쓴다.
문법 오진을 만들려고 target에 실제 문법 오류를 넣지 않는다. kind 라벨은 정답의 근거가 아니다.
`
...
- 이유 선택지 각각을 target·source·상황과 대조한다. 실제 주원인과 같은 뜻인 오답이 있거나,
  이유가 아닌 사실 확인 문장만 있으면 fail이다. 서로 다른 말투로 썼다는 이유로 구별하지 않는다.
