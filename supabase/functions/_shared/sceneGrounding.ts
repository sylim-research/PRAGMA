...
[P 판정 — 응답 화행]
거절·반대·불만처럼 상대의 제안·요청·결정에 응답하는 화행에서, 화자가 그 제안을 받아들일지 스스로 정한다는 사실은 P가 아니다.
제안·요청을 받은 사람은 지위와 무관하게 언제나 수락 여부를 정한다. P는 두 사람 사이의 직위와 평가·지시·자원 배분 권한으로만 판정한다.
예: 신입 사원이 다른 회사 이사의 공동 마케팅 제안을 거절한다 → 이사가 더 높으므로 speaker_lower다.
reason_ko와 p_evidence_ko에서는 코드 이름보다 「화자가 상대보다 낮다/같다/높다」를 먼저 쓰고, 그 문장이 코드 이름의 뜻과 일치하는지 확인한다.
