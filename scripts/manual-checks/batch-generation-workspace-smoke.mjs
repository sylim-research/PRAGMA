...
try {
  ...
  for (const width of [1440, 1280, 1024, 390]) {
    ...
    if (width === 1280) {
      const assemblyLink = page.locator("#batch-execution").getByRole("link", { name: "미션 조립으로", exact: true });
      await assemblyLink.scrollIntoViewIfNeeded();
      await expect(assemblyLink).toBeInViewport();
    }
  }
  ...
  const result = { status: "PASS", scope: "Local UI; production service requests blocked; no AI generation", checks: ["72/495/30 plans", "selection across pages", "preset resets selection", "no overflow at 1440/1280/1024/390", "desktop execution panel bottom actions accessible", "mobile execution shortcut"], screenshots: output };
