...
try {
  ...
  const queries=await Promise.all([
    ...
    db.from("content_review_runs").select("id,target_id,kind,content_hash,source_hash,rules,openai_review,claude_review,adjudication,approved_at,last_error,created_at,running_stage").eq("kind","mission").order("created_at",{ascending:false}).range(0,199),
  ...
  const slots=plan.plan.map((slot:any)=>{
    ...
    const review=reviews.find(r=>r.target_id===id)??null;
    const mission=row?.mission_content;
    const full=mission?.mpj_items?.length===5;
    const aiComplete=!!(review?.openai_review&&review?.claude_review&&review?.adjudication);
    const hasFindings=mission?.quality_check?.verdict==="fail"||review?.openai_review?.result?.verdict==="fail"||review?.claude_review?.result?.verdict==="fail";
    const status=!full?"미션 미완성":hasFindings?"판정 검토 필요":aiComplete?"교수자 판단 대기":"추가 검토 전";
    return {...slot,scenarioId:id??null,row,review,status,full,aiComplete,hasFindings,
      generationError:original.error??null,replacement:alternate??null,reviewError:review?.last_error??reviewed.slots[slot.ordinal]?.reviewError??null,
  ...
  const bundle={version:"course_content_review_book_v1",at,lifecycle:"candidate_for_professor_review",release:plan.release,planHash:plan.planHash,
