...
const prompts = [
  entry("core.scene_preflight.system", "장면 사전 검토 · 지시문", "core",
    "원문을 쓰기 전에 사건과 관계 조건(P·D·R)이 실제로 성립하는지 확인합니다.", S.CORE_SCENE_PREFLIGHT_PROMPT),
  ...
  entry("core.system.ko_zh", "상황 시나리오 생성 · 지시문 (한→중)", "core",
    "상황 설명과 원문만 만듭니다. 학습 문항은 이 단계에서 만들지 않습니다.", S.buildCoreSystemPrompt("ko_zh")),
  entry("core.system.zh_ko", "상황 시나리오 생성 · 지시문 (중→한)", "core",
    "한→중과 같은 구조에서 원문과 산출 언어만 바꿉니다.", S.buildCoreSystemPrompt("zh_ko")),
  entry("core.user.written", "상황 시나리오 생성 · 요청서 (번역)", "core",
    "직장·산업 등 생성 조건이 들어가는 자리입니다. 조건값은 호출마다 채워집니다.",
  ...
  entry("core.user.written.zh_ko", "상황 시나리오 생성 · 요청서 (중→한 번역)", "core",
    "화자가 직접 말하는 중국어 원문과 목표 화행을 시나리오 단계에서 고정합니다.",
    S.buildCoreUserPrompt({
      ...S.CORE_PROBE_BASE,
      direction: "zh_ko",
      source_modality: "written",
  ...
  entry("core.user.spoken.zh_ko", "상황 시나리오 생성 · 요청서 (중→한 통역)", "core",
    "중국어 화자(A)·한국어 청자(B)·통역하는 학습자(C)의 역할을 고정합니다.",
  ...
  entry("core.user.spoken.zh_ko.response_act", "상황 시나리오 생성 · 요청서 (중→한 응답 통역)", "core",
    "중국어 화자와 한국어 화자가 주고받는 대화쌍과 통역이 끼어드는 지점을 고정합니다.",
    S.buildCoreUserPrompt({ ...S.CORE_PROBE_BASE, direction: "zh_ko", source_modality: "spoken", is_response_act: true })),
  entry("core.user.response_act", "상황 시나리오 생성 · 요청서 (거절·반대 등 응답 화행)", "core",
    "응답 화행 앞에 오는 상대의 말(선행 발화)을 반드시 함께 만들게 합니다.",
  ...
  entry("core.user.source_repair", "상황 시나리오 생성 · 원문 분량 1회 교정", "core",
    "원문 길이나 문장 수가 기준을 벗어나면 내용은 그대로 두고 한 번 다시 고칩니다.",
  ...
  entry("core.user.preceding_turn_repair", "상황 시나리오 생성 · 선행 발화 언어 1회 교정", "core",
    "선행 발화가 목표 언어로 쓰이지 않았으면 내용과 역할은 그대로 두고 한 번 다시 고칩니다.",
  ...
  entry("core.user.bilingual_scene_repair", "상황 시나리오 생성 · 통역 장면 역할 1회 교정", "core",
    "통역 장면에서 화자·통역자·청자 역할이 겹치면 한 번 바로잡습니다.",
    S.buildCoreOutputRepairPrompt({
      originalUserPrompt: "PROBE_USER_PROMPT",
  ...
  entry("mission.system.spoken", "학습 미션 조립 · 지시문 (통역)", "mission",
    "말로 산출하는 통역 미션에 쓰입니다.", S.buildMissionSystemPrompt(PROBE_FEATURE, false, true, "ko_zh")),
  entry("mission.system.legacy_v4", "학습 미션 조립 · 이전 4문항 MJT 지시문", "mission",
    "초기 4문항 형식으로 만든 미션을 읽고 이어 쓰기 위한 호환용입니다.",
    S.buildMissionSystemPrompt(PROBE_FEATURE, false, false, "ko_zh", false)),
  entry("mission.user.contrast_plan", "학습 미션 조립 · 문항 대비 계획 요청서", "mission",
    "만들기 전에 화행 목표와 다섯 문항이 각각 판단할 초점을 먼저 정합니다.",
    S.buildMissionUserPrompt(probeMissionBody("ko_zh", "spoken", false))),
  entry("mission.item_lineage.system", "학습 미션 문항별 근거 연결 · 지시문", "review",
    "완성된 문항 문장을 규칙·위험 항목에 연결합니다. 승인 상태는 바꾸지 않습니다.",
    S.buildItemLineageSystemPrompt(PROBE_FEATURE.lineage_scope)),
  entry("core.user.learner_scene_repair", "상황 시나리오 생성 · 정답 단서 1회 교정", "core",
    "상황 사실은 두고, 정중성·완화처럼 답의 방향을 알려 주는 단서만 지웁니다.",
    S.buildCoreOutputRepairPrompt({
      originalUserPrompt: "PROBE_USER_PROMPT",
