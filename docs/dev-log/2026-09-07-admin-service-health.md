...
>
> 통째로 위에 올리면 매일 보는 운영 지표가 아래로 밀린다. 그래서 **항상 요약 한 줄로 접어 두고**
> 「펼쳐 보기」로 목록을 연다(PC 폭 1440에서 46px 한 줄). 콘텐츠 수치가 첫 화면의 주인공이라는
> 연구자 판단이다.
>
> 🔑 **요약 불은 「확인된 문제」에만 반응한다.** 점검 경로가 없어 판정하지 못한 항목(manual)은 불을
> 깎지 않는다 — 확인된 fail·warn이 없으면 **초록**이고, 문구에 어디를 직접 봐야 하는지 남긴다
> (`3개 정상 · OpenAI·Anthropic 직접 확인`). 「모르는 것」을 「나쁜 것」으로 칠하면 살아 있는 시스템이
> 죽은 것처럼 보인다. 실제 문제는 접혀 있어도 색과 이름으로 드러난다(`실패 1건 · OpenAI`, 붉은 점).
...
- `npm run typecheck` 통과 · eslint 통과 · `vitest` 신규 27건 통과 · `vite build --mode development` 통과.
