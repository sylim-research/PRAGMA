...
- 🔑 **「실패」와 「직접 확인」을 구분한다**(연구자 지적으로 추가). 점검 함수에 닿지 못했을 때
  ...
  **우리가 물어보지 못한 것**이지 제공자 상태를 아는 게 아니다. 이때는 회색 「직접 확인」 + 해당
  콘솔 링크를 준다. 빨간불은 우리가 고칠 수 있는 실제 문제(키 미등록·키 인증 실패·관리자 로그인
  필요)에만 켠다.
...
- `npm run typecheck` 통과 · eslint 통과 · `vitest` 신규 20건 통과 · `vite build --mode development` 통과.
...
- **미실시**: 관리자 로그인이 필요한 실제 화면 확인과 운영 배포. edge function `service-health`는 **배포 전**이라
  배포 전까지 OpenAI·Anthropic 두 줄은 회색 「직접 확인」 + 콘솔 링크로 나온다(빨간불 아님). 배포하면
