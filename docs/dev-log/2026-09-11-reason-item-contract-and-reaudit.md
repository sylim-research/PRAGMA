...
## 22. 승인 직전 P0 — content_review_v3 승인 계약 불일치 수정 · 2026-09-12

- 발견: 교수자 최종 승인 준비 중, 운영 DB의 `assert_content_review_base_ready`가 `criteria_version <> 'content_review_v2'`면 거부. 이 검사를 교수자 결정 저장·승인·`finalize_reviewed_mission`이 모두 먼저 호출하므로 20건(전부 v3) 승인 불가. `assert_content_review_ready`(수업자료 출처)·`guard_content_review_scenario`(reviewed 전환 트리거)·`get_approved_weekly_material`에도 v2 고정. 원인 = 17f68028(PR #140)이 앱 버전 상수만 v3로 올리고 DB 함수는 그대로.
- 계약 감사: v3는 **DB 승인 의미가 달라지지 않았다.** 17f68028의 변경은 Edge finalization 생성 기준(not_covered 표시, reason 계보 경로)과 finalization 재시도 제거뿐. DB의 해시·증거·결정·override·상태 전환 조건은 버전 문자열 외 동일하고, 모델 출력 검증은 이미 `criteria_version||':'||stage`로 버전 무관. 20건 모두 계보 `coverage_status`와 finalization이 일치(covered 8은 item_lineage 객체, not_covered 12는 계보도 not_covered) → `Covered mission requires finalized item lineage`에 걸리지 않음.
- 수정(`supabase/migrations/20260912090000_content_review_v3_approval.sql`, PR #144, main `ca5a5405`): `content_review_criteria_rank(text)`(v3=3·v2=2·그 외 0)를 지원 버전의 단일 기준으로 두고, 네 함수의 v2 문자열만 `rank > 0`(base_ready는 `= 0`이면 거부)으로 교체. 나머지 본문은 운영 정의를 그대로 옮김(정의 diff = 버전 조건 줄만). `get_approved_weekly_material`은 같은 원본의 여러 승인 중 최신 기준(v3)을 우선하도록 `ORDER BY rank DESC, approved_at DESC, created_at DESC LIMIT 1`. 데이터 변경 없음.
- 검증: DB 계약 테스트(pglite, 실제 migration 로드) **23/23 pass** — 기존 v2 19개(호환) + v3 4개(결정 저장·defer 차단·override 누락/짧은 사유 차단·정상 finalize와 계보 연결 / 해시 불일치·원본 변경 차단 / v4 거부와 v3 대조군 / 주간 자료의 미승인 v3 비노출·v3가 뒤늦은 v2 승인보다 우선) · typecheck pass · vitest 926 pass(0 fail) · CI pass.
- 운영 적용: `supabase db push`(dry-run으로 대상 1건 확인 후). 적용 뒤 운영 정의 재조회 — 네 함수 모두 rank 사용, v2 문자열은 rank 함수에만, base_ready에 검수·원본 해시 검사 유지, `validate_content_review_decisions`의 「revision or defer cannot be approved」와 `finalize_reviewed_mission`의 「unresolved critical AI issue requires a professor override」 유지(미수정). rank: v2=2, v3=3, v4·빈 값·null=0.
- w5-0 마지막 언어 수정(`c11-fix/content-fix.ts`): 「有我照片参展的公司内部展览」→「我有照片参展的公司内部展览」 12곳(MJT2~4 target·권장안·교정안). 상황·P/D/R·초점·대역·highlights·구조 불변 검증. rev 11, 미션 hash 4cd5148b, gate v24 warning(r1 possible, 기존과 동일), 검수 hash c61a7a64: 규칙 warning → finalization → Claude warning 1(MJT2 해설 「거리만 달라졌습니다」가 활동 변화를 빠뜨림, 기존 문구, accept·교수자 확인 없음) → adjudication 1 → next=professor.
- 상태: 20/20 next=professor, 승인 0. 편성·vertical slice 미실행.
