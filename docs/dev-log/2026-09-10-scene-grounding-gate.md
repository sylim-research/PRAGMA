...
### 2차 배포 — PDR 코드 오독 방어 두 줄 (같은 날)

1차 수정 배포 뒤에도 학생↔교수 셀에서 보류가 이어졌고, 이유가 두 종류의 **코드 오독**이었다.

- P를 거꾸로 읽음: 「교수가 상대이므로 speaker_higher」. 규칙은 `speaker_lower=화자가 낮음`이다.
- D를 일상 영어 뜻으로 읽음: 교수·학생을 「사적 친분이 없으니 distant」. 규칙의 distant는 **초면**만 가리킨다. 1차에서 `formal`→`distant`로 이름을 맞춘 뒤 이 오독이 나타났다 — 라벨이 일상 영어의 「거리감 있는」으로 읽힐 여지가 생긴 것이다.

`SCENE_GROUNDING_RULE`에 판정 기준을 바꾸지 않는 방어 문장 두 개를 넣었다: 「상대가 더 높은 사건은 speaker_higher가 아니라 speaker_lower다」, 「distant는 초면에만 쓴다. 교류 이력이 있으면 사적 친분이 없거나 격식을 차리는 사이여도 acquaintance다」. 스냅샷 재생성 후 typecheck·920 tests 통과, `generate-scenario` 재배포.

**결과: 부분적이다.** D 오독은 이후 재현되지 않았으나 P 오독은 배포 후에도 w5-1·w6-1에서 다시 나왔고, 「모든 축이 일치한다」고 서술하면서 `feasible:false`를 반환하는 응답도 나왔다. 두 칸은 재시도를 중단하고 시드 명시 여부를 연구자 판단으로 넘겼다(`docs/dev-log/2026-09-10-course-core-review.md`).

이 두 번의 배포로 프롬프트 스냅샷 해시가 두 번 바뀌었다. 콘텐츠 후보 ID는 `_03`을 유지했다(판정 기준이 아니라 코드 이름·오독 방어만 손댔으므로). 행별 `prompt_snapshot_hash`로 어느 판본에서 생성됐는지 구분된다.
