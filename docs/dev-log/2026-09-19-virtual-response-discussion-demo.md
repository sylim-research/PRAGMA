...
## 결정 (2026-09-19, Codex 검토·연구자 승인)

- 가상 분포는 논문용 시연으로만 유지한다. 운영 학급 응답 화면에는 넣지 않는다.
- 참고 판단과 최다 응답의 차이는 그대로 둔다. 판단 근거를 비교하는 토론 예시로 설명하고, 학습자 오류나 실제 경향으로 해석하지 않는다.
- 도판은 제5장 그림 63(`D-5.3-수업활용예시`)의 하위 패널로 통합한다. 새 그림 번호는 없다.
- 후속 개선(백로그): 토론 질문 세 개를 운영 「학급 응답 분포」 화면에 추가. 지금은 필수가 아니며, 구현 전에 UI 변경 안부터 승인받는다.
