...
- 연구자가 평가 범위 과장을 피하도록 후속 판정을 내려, 앞 절의 `언어 자연성` 제목 변경을 철회하고 준비·결과 카드 모두 `문법 정확성`으로 복원했다. 앞 절은 당시 지시 이력이며 이 절이 최종 상태다.
- question은 목표 언어를 유지한 `중국어 표현에 문법적인 문제가 없나요?` 형식으로 좁혔다. 현재 피드백 카드는 question을 직접 렌더링하지 않고 제목·본문을 표시하므로 새 질문 UI를 추가하지 않았다. internal key/schema/prompt와 실제 판정 범위는 변경하지 않았다.
- 이견 버튼·패널 제목은 `AI 피드백과 내 생각이 다르다면`으로 맞췄다. 이견 저장·유지/수정 분기 변경 없음.
- focused test: 최초 화면 검사에서 비노출 question을 찾는 단언 1건이 실패하여 해당 단언을 runtime 변환 검사로 옮겼다. 최종 2파일에서 관련 **3 passed / 6 filtered out(skipped)**. 제목·이견 표시와 language key/label/question·기존 판정 level을 확인했다. 전체 테스트·실제 AI·DB·운영 E2E 없음.
- 콘텐츠·hash·lesson_points·reference_alternatives·저장 계약 미수정. 연구 기록 추가 설계 변경 없이 이 dev-log에만 기록했다. 로컬 커밋까지만 수행하고 push/deploy하지 않는다.
