...
describe("admin dashboard", () => {
  it("keeps approval totals and review navigation without the removed action banner", async () => {
    show();
    expect(screen.queryByRole("region", { name: "지금 할 일" })).not.toBeInTheDocument();
    const approvedLink = screen.getAllByRole("link", { name: /교수자 최종 승인 완료/ })[0];
    await waitFor(() => expect(approvedLink.textContent).toContain("2"));
    expect(approvedLink).toHaveAttribute("href", "/admin/review");
    expect(screen.queryByText(/현재 보유본 기준/)).not.toBeInTheDocument();
  ...
  it("drops the duplicated production flow row and keeps no helper sentence", async () => {
    ...
    const records = screen.getByRole("link", { name: /편성 안 학습 수행 기록/ });
  ...
  it("keeps the four operation cards with account and record labels that do not imply real students", async () => {
    ...
    expect(assignments).toHaveAttribute("title", "승인 전 미션 1개 포함(승인 절차 도입 전 편성)");
    expect(screen.getByRole("link", { name: /가입 승인 학습자/ })).toHaveAttribute("href", "/admin/learners");
    const records = screen.getByRole("link", { name: /편성 안 학습 수행 기록/ });
    expect(records.textContent).toMatch(/편성 안 학습 수행 기록\s*1\s*건/);
    expect(records.textContent).toMatch(/편성 외 수행 2건 별도$/);
  });
