...
describe("local learner UX pilot", () => {
  ...
  it("runs five independent scenes, ungraded free correction and one new DCT without external writes or AI", () => {
    ...
    click("조정이 필요함"); click("답안 확인하기"); click("다음: 판단하고 고쳐 보기");
    click("조정이 필요함"); click("판단 확인하기");
    ...
    // Any initial judgment can proceed, and a non-reference answer remains the learner's answer.
    click("상황에 잘 맞음"); click("판단 남기고 직접 고치기");
    ...
    expect(snapshot.responses.A4).toEqual({ judgment: "appropriate", revisedText: freeAnswer });
    expect(snapshot.questIndex).toBe(4);
    // Only completed-step progress is restored; no full draft recovery machinery.
    view.unmount(); mount();
    const bands = ["상황에 맞음", "지나치게 우회적", "상황에 맞음", "너무 직접적"];
    ...
    click("네 표현 확인하기"); click("다음: 직접 옮겨 보기"); click("직접 옮겨 보기");
    ...
    expect(completed.responses.A5.candidateJudgments).toEqual({ a: "appropriate", b: "too_indirect", c: "appropriate", d: "too_direct" });
    ...
    click("처음부터 다시 보기");
    expect(screen.getByRole("button", { name: /표현 판단 시작하기/ })).toBeInTheDocument();
