...
const OpeningSession = forwardRef<HTMLDivElement, { opening: WeeklyOpening; onClose: () => void }>(function OpeningSession({ opening, onClose }, ref) {
  ...
  return <Dialog.Content ref={ref} className="opening-room" onKeyDown={(event) => {
    ...
    <div ref={scrollRef} className="opening-scroll">
      ...
      <Dialog.Description className="sr-only">교수자가 진행하는 도입 활동. 진행 시간은 수업에 맞게 조정합니다. 좌우 방향키로 이동하고 Escape로 닫습니다. 응답은 저장하거나 채점하지 않습니다.</Dialog.Description>
      <main className={`opening-stage opening-stage-${step}`} key={step} aria-live="polite">
        ...
        <h1>{step === 0 ? opening.question : step === 1 ? "지금, 여러분의 생각은?" : step === 2 ? "이 사실을 알게 된다면?" : "어떤 근거로 판단을 유지하거나 바꿨나요?"}</h1>
        {step <= 1 && <>
          ...
          {step === 0 && opening.centralQuestion && <details className="opening-support"><summary>이번 주 중심 질문 · {opening.centralQuestion}</summary><p>{opening.questionGuidance}</p></details>}
...
export function WeeklyOpeningLesson({ opening, onStart }: { opening: WeeklyOpening; onStart?: () => void }) {
  ...
  return <section className="opening-launcher" aria-label="주차 도입 수업">
    <div className="opening-launcher-copy"><p className="opening-label">수업의 첫 10–15분 · 시간은 조정 가능</p><h2>한 장면에서 시작하는 오늘의 수업</h2><p>{opening.contextLabel}</p><p className="opening-launcher-goal">학습 초점 · {opening.goals[0] ?? opening.centralQuestion ?? "주차 학습목표를 먼저 설정해 주세요."}</p></div>
