...
import { CENTRAL_QUESTION_GUIDANCE, weekActivityLabel, weekCentralQuestion } from "./weekGuidance";
...
export function buildWeeklyOpening(outline: LearnerCourse["outline"], week: LearnerCourseWeek): WeeklyOpening {
  ...
  const candidateChannel = week.channel ?? assigned?.context?.channel;
  const channel = candidateChannel && Object.prototype.hasOwnProperty.call(CHANNEL_TO_MODE, candidateChannel) ? candidateChannel as ChannelUI : null;
  const note = buildWeeklyLearnerNote(week, direction === "zh_ko" ? "zh_ko" : "ko_zh");
  const goals = [...(note.competencyFocus ? [note.competencyFocus] : []), ...note.canDos];
  ...
  return {
    ...
    notice: assigned && !completeContext(planned)
      ? "교수자 검토용 별도 예시 · 편성 미션 하나의 관계·거리·부담을 참고했습니다. 두 미션의 공통 조건이 아니며, 장면과 단서의 적합성을 확인하세요."
      : base.notice,
    role: mode === "stt_interpreting" ? `원발화자 A → 청자 B · 여러분은 통역사 C입니다. 교수자가 ${source.language === "ko" ? "한국어" : "중국어"} 원문을 읽고, 아래 통역 초안을 함께 검토합니다.` : channel ? `여러분이 보내려는 ${CHANNEL_UI[channel]}의 원문과 번역 초안입니다.` : "여러분이 전달하려는 원문과 번역 초안입니다.",
    source, rendering,
