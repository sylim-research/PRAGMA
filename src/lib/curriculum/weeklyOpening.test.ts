...
describe("교과목·주차 도입 자료", () => {
  ...
  it("미지정 또는 잘못된 조건을 임의로 보완하지 않는다", () => {
    ...
    for (const patch of [{ pdr_distance: null }, { channel: "phone" as const }, { can_do: [], competency_focus: null }]) expect(buildWeeklyOpening(openingOutline, { ...openingWeek, ...patch }).status).toBe("unavailable");
  });
